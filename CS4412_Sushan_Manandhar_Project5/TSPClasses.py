...
class TSPState:
    def __init__(self, cost, path, table, depth, cityIndex, cities):
        self.cost = cost
        self.path = path  # path list for BSSF
        self.table = table  # City-city cost 2d numpy array
        self.depth = depth
        self.cityIndex = cityIndex
        self.cities = cities
        self.length = len(cities)

    def __lt__(self, other):  # Checks if it has higher priority
        if self.depth > other.depth:
            return True
        if self.depth == other.depth and self.cost < other.cost:
            return True
        return False

    def reduce(self):
        # For row
        mins = np.amin(self.table, axis=1)
        for i in range(len(mins)):
            min = mins[i]
            if min != math.inf:
                self.cost += min
                for j in range(len(self.table)):
                    self.table[i, j] = self.table[i, j] - min
        # For column
        colMins = np.amin(self.table, axis=0)
        for i in range(len(colMins)):
            min = colMins[i]
            if min != math.inf:
                self.cost += min
                for j in range(len(self.table)):
                    self.table[j, i] = self.table[j, i] - min

    def getChildren(self):
        childrenStates = []
        for i in range(self.length):
            cell = self.table[self.cityIndex][i]
            if cell != math.inf: # if true then generates child
                cost = self.cost + cell
                depth = self.depth + 1
                path = self.path.copy()
                path.append(self.getCity(i))
                # Table builder
                childTable = self.table.copy()
                childTable[self.cityIndex] = [math.inf] * self.length
                childTable[:, i] = [math.inf] * self.length
                childTable[self.cityIndex, i] = math.inf
                childTable[i, self.cityIndex] = math.inf
                child = TSPState(cost, path, childTable, depth, i, self.cities)
                childrenStates.append(child)
